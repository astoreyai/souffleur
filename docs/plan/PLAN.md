...
- **Phase 2 — Desktop overlay surface.** ✅ **Built + verified on Linux** (see `PHASE2_RESULTS.md`): Tauri v2 frameless/transparent/always-on-top/click-through overlay with `contentProtected` and an honest per-OS capability badge driven by a real Rust `capture_status` command (Windows 🟢 hidden / macOS 15+ 🟠 visible / Linux 🟠 visible). Runs on `:0` (transparent over the desktop, drives the daemon) and screenshotted on Xvfb. *Exit caveat:* the real screen-capture **exclusion** is Windows-only and is the user's test on a Windows host; Linux/macOS-15+ are no-ops, reported honestly not hidden.
- **Phase 3 — Smart-glasses *display* (XREAL Air).** The MentraOS/Frame glasses-**SDK** surfaces were **removed (2026-06-21)** — hardware-blocked and not the path forward. XREAL Air (and similar birdbath glasses) are **USB-C DisplayPort monitors**, not BLE/SDK devices: they appear as an ordinary 1080p external display, so the existing phone/overlay web surface renders on them at monitor latency — and because the lens is a *separate display*, it is inherently off any shared window/primary screen (the same privacy property as the phone). *Plan:* a display-tuned surface (large, high-contrast text for a floating 1080p screen) + a one-command launcher to place a surface on the XREAL display. No SDK required.
