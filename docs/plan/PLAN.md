...
- **Phase 3 — Smart glasses.** ⏳ **Code built + verified; live-on-lens blocked on hardware** (see `PHASE3_RESULTS.md`). MentraOS surface (real `@mentra/sdk` v2.1.29 `AppServer` → `session.layouts.*`, type-checks against the live API, 12 bridge tests) + Frame fully-open PC-direct reference (real `frame-sdk` 1.2.4, 10 bridge tests, imports clean). Both are thin Coach Protocol display surfaces. *Exit NOT met:* live prompts on real glasses + on-hardware chars/line + latency need physical glasses (none on the dev box) — the user's test. Display geometry defaults are placeholders to measure + override.
